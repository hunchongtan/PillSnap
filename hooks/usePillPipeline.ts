...
export function usePillPipeline() {
  ...
  const run = useCallback(async (file: File) => {
    ...
    try {
      ...
      const predictions = (segJson.predictions || []).filter(
        (p) => (p.confidence ?? 0) >= 0.6
      );
      if (!predictions.length)
        throw new Error("No pills detected with confidence ≥ 0.6");
        ...
        mapped.map(async (d) => {
          try {
            ...
            const attributes = {
              ...
              thickness_mm:
                typeof a.thickness_mm === "number" ? a.thickness_mm : 0,
              front_imprint:
                a.front_imprint && a.front_imprint !== "unclear"
                  ? a.front_imprint
                  : "",
              back_imprint:
            ...
            setDets((prev) =>
              prev.map((x) =>
                x.id === d.id ? { ...x, attributes, loading: false } : x
              )
            );
          } catch (e) {
            setDets((prev) =>
