...
export async function extractPillAttributes(imageFile: File): Promise<VisionAnalysisResult> {
  try {
    console.log("[v0] Starting base64 conversion...")
    const base64Image = await fileToBase64(imageFile)
    console.log("[v0] Base64 conversion complete.")

    console.log("[v0] Sending request to OpenAI Vision API...")
    const response = await fetch("/api/openai/analyze-pill", {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      console.error("[v0] OpenAI Vision API error:", errorText)
      throw new Error(`OpenAI Vision API error: ${response.statusText}`)
    }

    const result = await response.json()
    console.log("[v0] OpenAI Vision API response:", result)
    return result as VisionAnalysisResult
  } catch (error) {
    console.error("[v0] OpenAI Vision analysis error:", error)
