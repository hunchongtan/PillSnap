...
export async function segmentPillImage(imageFile: File): Promise<ProcessedPillImage> {
  try {
...
export function drawBoundingBoxes(
  ...
  detections.forEach((detection) => {
    ...
    ctx.fillStyle = "#6366f1"
    ctx.fillRect(x - width / 2, y - height / 2 - textHeight, textMetrics.width + 8, textHeight)
