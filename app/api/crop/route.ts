...
async function readJson(request: NextRequest): Promise<{
  ...
  const box = boxParsed.data as Box;
  const paddingPct =
    typeof body.paddingPct === "number" ? body.paddingPct : 0.06;
  ...
  try {
    ...
    if (!input) {
      return NextResponse.json(
        { error: "Missing image or bounding box" },
        { status: 400 }
      );
    }
    ...
    let left = isTopLeft
      ? Math.round(box.x - padX)
      : Math.round(box.x - box.width / 2 - padX);
    let top = isTopLeft
      ? Math.round(box.y - padY)
      : Math.round(box.y - box.height / 2 - padY);
