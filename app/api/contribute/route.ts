...
export async function POST(request: NextRequest) {
  try {
    ...
    if (backImage && backImageName) {
      const { data, error: backError } = await supabase.storage.from("pill-images").upload(backImageName, backImage)
    ...
    const backUrl = backUpload ? supabase.storage.from("pill-images").getPublicUrl(backUpload.path).data : null
